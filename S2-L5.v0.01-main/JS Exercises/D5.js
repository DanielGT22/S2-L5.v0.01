...
/* ESERCIZIO 4
    Scrivi del codice per spostare il primo elemento dall'array "pets"
     in ultima posizione.
*/
const pets2 = pets.slice(1);
pets2.push(pets[0]);
console.log(pets);

//non so perchè a me si invertono piuttosto di scalare, da rivedere//
...
/* ESERCIZIO 6
    ...
    Successivamente, rimuovi l'ultimo elemento della proprietà "trims" da ogni
     auto.
*/
...
const justTrims = [];
for (let i = 0; i < cars.length; i++) {
  let trims = cars[i].trims.splice(0);
...
/* ESERCIZIO 8
    ...
    "color" ha valore "b", mostra in console "Fizz". Altrimenti, mostra in console 
    "Buzz".
*/
for (let i = 0; i < cars.length; i++) {
...
let y = 0;
let nostraParola = "pineapple";
let parola = nostraParola.split;
let risultato10 = [];

switch ("alfa") {
  case charactersArray[0] === parola[y]:
    risultato10.push(parola[y]);
    y++;
...
// da ripasare, potrei scrivere tutte le lettere ma pressumo che ci 
sia un modo più facile per farlo//
